...
class TestRDD:
    def test_recovers_true_late(self):
        df = make_data()
        result = RDD(make_dag(), treatment="treatment", running_var="score", cutoff=CUTOFF, outcome="outcome").fit(df)
    ...
    def test_bandwidth_restricts_data(self):
        df = make_data()
        result_full = RDD(make_dag(), treatment="treatment", running_var="score", cutoff=CUTOFF, outcome="outcome").fit(
            df
        )
        result_bw = RDD(
            make_dag(), treatment="treatment", running_var="score", cutoff=CUTOFF, outcome="outcome", bandwidth=0.5
        ).fit(df)
    ...
    def test_running_var_not_in_dag_raises(self):
        ...
        with pytest.raises(ValueError, match="Running variable"):
            RDD(dag, treatment="treatment", running_var="score", cutoff=CUTOFF, outcome="outcome")
    ...
    def test_overlapping_variables_raises(self):
        ...
        with pytest.raises(ValueError, match="different variables"):
            RDD(dag, treatment="treatment", running_var="score", cutoff=CUTOFF, outcome="treatment")
    ...
    def test_missing_running_var_column_raises(self):
        ...
        with pytest.raises(ValueError, match="Running variable column"):
            RDD(make_dag(), treatment="treatment", running_var="score", cutoff=CUTOFF, outcome="outcome").fit(df)
